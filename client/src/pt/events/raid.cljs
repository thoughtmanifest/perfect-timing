...
(defn- format-model
  [model]
  (->> model
       (map (fn [[fight-id predictions]]
              {:id (js/parseInt (name fight-id))
               :predictions predictions}))
       (store-by-id)))

(defn get-raid-model-success
  [{:keys [db]} [event-name raid-id model]]
  {:db (update-in db [:raids raid-id]
                  #(merge-with merge %
                               (format-model model)))})

(reg-event-fx :raid/get-raid-model-success get-raid-model-success)

(defn get-raid-model-failure
  [{:keys [db]} [event-name err]]
  {:db db})

(reg-event-fx :raid/get-raid-model-failure get-raid-model-failure)
...
(defn get-raid-model
  ...
  {:db db
   :request {:name event-name
             :method :get
             :uri (str "/output/" raid-id ".json")
             :response-format :json
             :on-success [:raid/get-raid-model-success raid-id]
             :on-failure [:raid/get-raid-model-failure]}})

(reg-event-fx :raid/get-raid-model get-raid-model)
